...
import { login, chat, getOrders, createOrder, transcribeAudio, synthesizeSpeech, downloadOrderPdf } from './services/api';
import './index.css';
...
function App() {
  ...
  // TTS State (Hindi voice replies; toggle to mute)
  const [ttsEnabled, setTtsEnabled] = useState(true);
  const [ttsNotice, setTtsNotice] = useState(null);       // friendly TTS error message
  const [pendingTtsUrl, setPendingTtsUrl] = useState(null); // blob URL blocked by autoplay policy
  ...
  // Speak the assistant's reply aloud — Hindi only for the demo, gated by the mute toggle.
  const playTTS = async (text, language) => {
    ...
    try {
      ...
      try {
        ...
        // Autoplay policy needs a user gesture — defer the reply to a click.
        // Keep this new URL alive for playPendingTts; only revoke the previous one.
  ...
  const sendMessage = async (textToSend) => {
    ...
    try {
      ...
      // Speak the assistant's latest reply (Hindi only, toggle-gated)
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      recorder.onstop = async () => {
        ...
        try {
          // POST audio to backend-core /api/voice/transcribe with language hint if available
          ...
          // Low-confidence STT (garbled audio) → friendly re-prompt, never the chat pipeline.
  ...
  // Download an order's PDF via the authed endpoint, then open the blob locally.
      ...
      <div className="panel">
        <div className="panel-header">
          ...
          <div className="flex items-center gap-2">
            ...
            {/* TTS Mute Toggle (Hindi voice replies) */}
            <button 
              type="button"
          ...
          {pendingTtsUrl && !ttsNotice && (
            <div className="voice-status-bar tts-pending">
              ...
              <button
                ...
                className="outline"
                style={{ padding: '2px 6px', fontSize: '11px', background: 'transparent', border: 'none', boxShadow: 'none' }}
                onClick={() => { setPendingTtsUrl((prev) => { if (prev) URL.revokeObjectURL(prev); return null; }); }}
              >
                <X size={14} />
              </button>
            </div>
