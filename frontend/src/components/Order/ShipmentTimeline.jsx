...
export default function ShipmentTimeline({ orderId }) {
  const cleanId = sanitizeOrderId(orderId);
  ...
  // prevent setState on unmounted — cancelled flag
  useEffect(() => {
  ...
  const fetchAll = useCallback(async () => {
    ...
    // backoff: skip next tick on 429/5xx
    ...
    if (isFirstLoad && !cancelledRef.current) setLoading(true);
    ...
    try {
      ...
      await Promise.all(arr.map(async (s) => {
        ...
        try {
          ...
          if (!cancelledRef.current) next[tn] = [];
        }
      }));
      if (cancelledRef.current || controller?.signal?.aborted) return;
      setEventsByTn(next);
      ...
      if (cancelledRef.current || controller?.signal?.aborted) return;
      // AbortError is not a real error — ignore
      ...
      if (!cancelledRef.current) {
        ...
        if (isFirstLoad) setShipments([]);
      }
    } finally {
      if (!cancelledRef.current && !controller?.signal?.aborted) setLoading(false);
  ...
  // 3s polling — clearInterval on unmount handled by usePolling; backoff skip handled inside fetchAll + hook
  const refresh = usePolling(fetchAll, 3000);
    ...
    return (
      <div className="bg-white rounded-xl border border-[#E1E7DF] p-6">
        ...
        <div className="py-8 text-center">
          ...
          <p className="font-['Figtree'] text-sm text-[#6B7568]">Loading shipments for {String(cleanId).slice(0, 8)}…</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="bg-white rounded-xl border border-[#E1E7DF] p-6">
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={handleRefresh} aria-label="Refresh timeline" className="p-2 rounded-lg border border-[#E1E7DF] hover:bg-[#F8FAF7]"><RefreshCw className="w-4 h-4" /></button>
        </div>
        <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 flex gap-2 font-['Figtree'] text-sm text-amber-800">
          ...
          <span>{error} — GET /tracking/orders/{cleanId}/shipments</span>
        </div>
        <button onClick={handleRefresh} className="mt-3 inline-flex items-center gap-1.5 px-4 py-2 bg-[#6FAF6F] text-white rounded-lg font-['Figtree'] text-sm font-medium hover:bg-[#5A9A5A]">
  ...
  if (!shipments.length) {
    return (
      <div className="bg-white rounded-xl border border-[#E1E7DF] p-6">
        <div className="flex items-center justify-between mb-3">
          <h3 className="font-['Fraunces'] text-lg font-semibold text-[#1B2E1B]">Tracking Timeline — Per Parcel</h3>
          <button onClick={handleRefresh} aria-label="Refresh timeline" className="p-1.5 rounded-lg border border-[#E1E7DF] hover:bg-[#F8FAF7]"><RefreshCw className="w-3.5 h-3.5" /></button>
        </div>
        <div className="rounded-lg border border-[#E1E7DF] bg-[#F8FAF7] p-6 text-center">
          <Truck className="w-8 h-8 text-[#A8C3A0] mx-auto mb-2" />
          <p className="font-['Figtree'] text-sm text-[#6B7568]">No shipments yet — booking triggers timeline</p>
          <p className="font-['Figtree'] text-xs text-[#6B7568] mt-1">Tracking timeline will appear per parcel after booking. Polls GET /tracking/orders/{String(cleanId).slice(0, 8)}/shipments → GET /tracking/shipments/{`{tn}`}/events.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-xl border border-[#E1E7DF] p-6">
      <div className="flex items-center justify-between mb-4">
        <h3 className="font-['Fraunces'] text-lg font-semibold text-[#1B2E1B]">Tracking Timeline — Per Parcel</h3>
        <span className="px-2 py-1 bg-[#F8FAF7] border border-[#E1E7DF] rounded-lg font-['Figtree'] text-xs text-[#6B7568]">{shipments.length} parcel(s) · GET /tracking/orders/{String(cleanId).slice(0, 8)}/shipments</span>
      </div>
      ...
      <div className="mt-4 flex justify-end">
        <button onClick={handleRefresh} aria-label="Refresh timeline" className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-[#E1E7DF] bg-white font-['Figtree'] text-xs text-[#1B2E1B] hover:bg-[#F8FAF7]">
